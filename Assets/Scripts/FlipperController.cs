        ...
        if(Input.touchCount > 0) {
            ...
            for (int i = 0; i < Input.touchCount; i++) {
                if (myTouches[i].phase == TouchPhase.Began) {
                    if (myTouches[i].position.x < center && tag == "LeftFlipperTag") {
                        SetAngle(this.flickAngle);
                    }
                    if (myTouches[i].position.x > center && tag == "RightFlipperTag") {
                        SetAngle(this.flickAngle);
                    }
                }
                
                if (myTouches[i].phase == TouchPhase.Ended && tag == "LeftFlipperTag") {
                    SetAngle(this.defaultAngle);
                }
                if (myTouches[i].phase == TouchPhase.Ended && tag == "RightFlipperTag") {
                    SetAngle(this.defaultAngle);
                }
            }
        }
